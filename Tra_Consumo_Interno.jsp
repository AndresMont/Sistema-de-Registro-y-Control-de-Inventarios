...
<html>
            ...
            function campos() {
                ...
                if ( d.value !== "" & h.value !== "0" & i!=="") {
                    ...
                    if (h.value === "0") {
                        ...
                        alert('Por favor, Ingrese la observación del consumo.');
                    }
                    return false;
                }
            }
    ...
    <sql:query var="resultado" dataSource="${con}">
        SELECT * FROM INVENTARIOSPI.INV_DOCUMENTOS WHERE DOC_TIPO_DOCUMENTO = 'CO'
    </sql:query>
       ...
       document.getElementById("uni").value = vaub;
        ...
        <c:if test="${param.modificar != null}">
            ...
            <sql:update var="gua" dataSource="${con}">
                ...
                DOC_OBSERVACIONES,
                DOC_TIPO_DOCUMENTO) 
                VALUES ('${cont+1}', '1','${param.bod}', '${suc_id}', '${usu_id}','1','${param.num}', STR_TO_DATE('${param.fec}', '%d/%m/%Y'), '${param.hor}', '${param.ite}','CONSUMO INTERNO.# ${param.num} ${param.obs}', 'CO');
            </sql:update>
            <script>
                function extraer() {
                <sql:query var="counts" dataSource="${con}">
...
//****************************************************************RELACIONES**********************************************************// 
            ...
            <form action="Tra_Consumo_Interno.jsp" method='post' >
