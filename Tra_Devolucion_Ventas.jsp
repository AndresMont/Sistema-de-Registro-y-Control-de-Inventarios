...
<html>
    <head>
        ...
        <script type="text/javascript">
            function campos() {
                ...
                var c = document.getElementById("cli");
                ...
                if (c.value !== "0" & d.value !== "" & e.value !== "" & f.value !== "" & g.value !== "" & h.value !== "0" & i.value !== "") {
                    return true;
                } else {
                    if (c.value === "0") {
                        alert('Por favor, Seleccione el nombre del cliente');
                    }
                    if (g.value === "") {
    ...
    <sql:query var="resultado" dataSource="${con}">
        SELECT * FROM INVENTARIOSPI.INV_DOCUMENTOS WHERE DOC_TIPO_DOCUMENTO = 'DV'
    </sql:query>
    ...
    <center>
        ...
        <c:if test="${param.modificar == null}">
            ...
            <form method="post" id="form" name="forma" onSubmit="return campos();">
                ...
                <table border="0" cellspacing="8">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>Cliente*:</td>
                            <td> <select name="cli" id="cli"  style=" width:520px; " disabled>
                                    <option value="0" >--SELECCIONE--</option>
                                    <c:forEach var="cols" items="${clientes.rows}"> 
                                        <option value="${cols.CLP_ID}"  >${cols.CLP_DNI} ${cols.CLP_PRIMER_APELLIDO} ${cols.CLP_SEGUNDO_APELLIDO} ${cols.CLP_PRIMER_NOMBRE} ${cols.CLP_SEGUNDO_NOMBRE} </option>
                                    </c:forEach>
                                </select> 
                            </td>
                            <td>Número de factura*:</td>
                            <td> <select name="num1" id="num1"  style=" width:520px; " disabled>
                                    <option value="0" >--SELECCIONE--</option>
                        ...
                        <tr>
                            <td>Observación*:</td>
                            <td><input type="text" name="obs" size="40" id="obs" disabled onkeyup="mayusculas();" ></td>
                            <td>Total:</td>
                            <td><input type="text" name="tot" size="40" id="tot" maxlength="20" disabled ></td>
                            <td><input type="hidden" name="id" id="id_tab" /></td>
                    <input type="hidden" value="${param.usuario}" name="usuar" >
                    </tr>
                ...
                <table >
                    <tr>
                        ...
                        <td><input type="submit" name="guardar" value="Guardar"  id="gua" class="botones" onmousedown="habilitar();"  disabled /></td>
                ...
                <table cellspacing="8"  border="0" id="bloque2">
                    <tbody>
                        <tr>
                            <td>Cantidad*:</td>
                            <td><input type="number" min="1" max="5" name="can" size="40" id="can" disabled onchange="cantidad()" onkeypress="return isNumberKey(event);" ></td>
                            <td>Detalle*:</td>
                            <td><select name="det" id="det"  style=" width:520px; " onclick="comprobar()" disabled >
                    ...
                    <input type="hidden" name="aux1" id="aux2" >
                    <input type="hidden" name="aux1" id="aux3" >
                    <input type="hidden" name="bode" id="bode" >
                    </tr>
                    </tbody>
                </table>
                <br>
            ...
            <script>
                ...
                function comprobar() {
                <%--     var x = document.getElementById("det").getElementsByTagName("option").getAttribute("disabled").length;
                     alert(x);
                     if (document.getElementById("det").getElementsByTagName("option").getAttribute("disabled") === (document.getElementById("det").length)-) {
                ...
                function auxiliar2(bodeg) {
                    document.getElementById("bode").value = bodeg;
                ...
                function agregar() {
                    ...
                    if (a > com) {
                        ...
                        var a = document.getElementById("val").value = "gua";
                        var a = document.getElementById("can").value;
                        ...
                        if (a === "" || document.getElementById("det").value === "0") {
                            if (a < 1 && a !== "") {
                            ...
                            var t4 = ((parseFloat(v) || 0) + (parseFloat(w) || 0)).toFixed(2);
                            ...
                            document.getElementById("vusb" + p).value = document.getElementById("aux3").value;
                ...
                function auxiliar(can, vau, vaug, vaub) {
                    ...
                    document.getElementById("aux3").value = vaub;
                    var a = document.getElementById("can").value;
                    var b = document.getElementById("uni").value;
                    var t = (a * b).toFixed(2);
                    document.getElementById("totp").value = t;
                ...
                function quit() {
                    ...
                    //SUBTOTAL
                    var y = document.getElementById("sub").value;
                    var t2 = ((parseFloat(y) || 0) - parseFloat(b)).toFixed(2);
                    document.getElementById("sub").value = t2;
                    //I.V.A
                    var z = document.getElementById("sub").value;
                    var t3 = ((parseFloat(z) || 0) * 0.14).toFixed(2);
        ...
        <c:if test="${param.modificar != null}">
            ...
            <sql:update var="gua" dataSource="${con}">
                ...
                DOC_OBSERVACIONES,
                DOC_TIPO_DOCUMENTO) 
                VALUES ('${cont+1}', '1' ,'${param.bode}', '${suc_id}','${usu_id}','${param.cli}','${param.num1}', STR_TO_DATE('${param.fec}', '%d/%m/%Y'), '${param.hor}', '${param.ite}', '${param.sub}', '${param.iva}', '${param.tot}','DEVOLUCIÓN EN VENTA. #FACTURA ${param.num1} ${param.obs}', 'DV');
            </sql:update>
            <script>
                function extraer() {
...
//**********************************PONDERACION BODEGAS**************************************
                ...
                <sql:update  var="saldosb" dataSource="${con}">
                    ...
                    INVENTARIOSPI.INV_SALDOS_POR_BODEGA(
                            ...
                            SPB_VALOR_UNITARIO,
                            SPB_VALOR_TOTAL)
                    VALUES('${SPB_ID+1}', '${param.bode}', '${param.det1}', '${SAL_CAN+param.can1}', '${unitario}', '${SAL_VTOT+param.vtsb1}')
                    ...
                    , ('${SPB_ID+2}', '${param.bode}', '${param.det2}', '${SAL_CAN+param.can2}', '${unitario1}', '${SAL_VTOT+param.vtsb2}')
                        <c:if test="${contador>2}">
                    ...
                    , ('${SPB_ID+3}', '${param.bode}', '${param.det3}', '${SAL_CAN+param.can3}', '${unitario2}', '${SAL_VTOT+param.vtsb3}')
                            <c:if test="${contador>3}">
                    ...
                    , ('${SPB_ID+4}', '${param.bode}', '${param.det4}', '${SAL_CAN+param.can4}', '${unitario3}', '${SAL_VTOT+param.vtsb4}')
                                <c:if test="${contador>4}">
                    ...
                    , ('${SPB_ID+5}', '${param.bode}', '${param.det5}', '${SAL_CAN+param.can5}', '${unitario4}', '${SAL_VTOT+param.vtsb5}')
                                    <c:if test="${contador>5}">
               ...
               <sql:query var="SALDOSB" dataSource="${con}">
                    ...
                    , ('${SPB_ID+6}', '${param.bode}', '${param.det6}', '${SAL_CAN+param.can6}', '${unitario5}', '${SAL_VTOT+param.vtsb6}')
                                    </c:if>
                                </c:if>
                            </c:if>
                        </c:if>
...
//****************************************************************RELACIONES**********************************************************// 
            ...
            <form action="Tra_Devolucion_Ventas.jsp" method='post' >
